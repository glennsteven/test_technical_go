...
func TestStructToMap(t *testing.T) {
	...
	for _, x := range testCase {
		...
		if fmt.Sprintf("%T", x.Error) == fmt.Sprintf("%T", e) {
			...
			t.Logf("expected '%v', actual '%v'", x.Error, e)
		} else {
			t.Errorf("expected '%v',actual '%v'", x.Error, e)
		}

		if len(c) == x.CountCol {
			t.Logf("expected %v,actual %v", x.CountCol, len(c))
		} else {
			t.Errorf("expected %v,actual %v", x.CountCol, len(c))
		}
	}
}
...
func TestToColumnsValues(t *testing.T) {
	...
	for _, x := range testCase {
		...
		if len(v) == x.CountCol {
			t.Logf("expected %v, actual %v", x.CountCol, len(v))
		} else {
			t.Errorf("expected %v, actual %v", x.CountCol, len(v))
		}
